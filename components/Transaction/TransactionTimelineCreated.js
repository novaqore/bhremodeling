...
const TransactionTimelineCreated = ({ request, company, complete }) => {
    ...
    const handleDateSelect = (date) => {
        // Convert the datetime-local value to Unix timestamp in milliseconds
    ...
    // Convert Unix timestamp to datetime-local format for the input default value
        ...
        <>
            ...
            >
                <div className={`w-14 h-14 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-200 group-hover:scale-105 ${
                    complete ? 'bg-green-100 group-hover:bg-green-200' : 'bg-blue-50 group-hover:bg-blue-100'
                }`}>
                    {complete ? (
                        <CheckSquare className="w-6 h-6 text-green-600 transition-all duration-200 group-hover:scale-110" />
                    ) : (
                        <CalendarCheck className={`w-6 h-6 text-blue-500 transition-all duration-200 group-hover:scale-110`} />
                    )}
                </div>
                <div className="flex-1 pt-2">
                    <h3 className="font-medium text-gray-900 group-hover:text-blue-600 transition-colors duration-200">Request Created</h3>
                    ...
                    <div className="bg-white rounded-xl p-8 max-w-md w-full mx-4 shadow-2xl transform transition-all">
                        ...
                        <div className="flex justify-between items-center mb-6">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="text-xl font-semibold text-gray-900">Update Creation Date</h3>
                                    <p className="text-sm text-gray-500 mt-1">Set when this request was created</p>
                                </div>
                            </div>
                            <button 
