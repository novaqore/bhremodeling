...
const TransactionTimelineCheckReceived = ({ request, company, complete }) => {
    ...
    const isPending = !request.checkReceivedDate;
            ...
            >
                <div className={`w-14 h-14 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-200 group-hover:scale-105 ${
                    ...
                    {complete ? (
                        ...
                        <Hash className={`w-6 h-6 transition-all duration-200 group-hover:scale-110 ${
                            isPending ? 'text-gray-400' : 'text-blue-500'
                        }`} />
                    )}
                </div>
                <div className="flex-1 pt-2">
                    <h3 className="font-medium text-gray-900 group-hover:text-blue-600 transition-colors duration-200">Check Received</h3>
                    <p className="text-sm text-gray-500 mt-1">
                        {isPending ? 'Pending' : formatDate(request.checkReceivedDate)}
                    </p>
                    <div className="mt-2 text-sm text-gray-600">Check from {company?.name || 'company'} collected</div>
                </div>
                    ...
                    <div className="bg-white rounded-xl p-8 max-w-md w-full mx-4 shadow-2xl transform transition-all">
                        ...
                        <div className="flex justify-between items-center mb-6">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="text-xl font-semibold text-gray-900">Check Received</h3>
                                    <p className="text-sm text-gray-500 mt-1">Set the date and time when the check was received</p>
                                </div>
                            </div>
                            <button 
                        ...
                        <div className="mb-8">
                            ...
                            <input
                                ...
                                defaultValue={request.checkReceivedDate || ''}
                            />
                        </div>
...
export default TransactionTimelineCheckReceived;